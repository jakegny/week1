...
template sudoku() {
    ...
    //[assignment] hint: you will need to initialize your RangeProof components here
		component rangeProofPuzzle[9][9]; // = RangeProof(32);
		component rangeProofSoution[9][9]; // = RangeProof(32);
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            rangeProofSoution[i][j] = RangeProof(32);
						rangeProofSoution[i][j].in <== solution[i][j];
						rangeProofSoution[i][j].range[0] <== 0;
            rangeProofSoution[i][j].range[1] <== 9;
						assert(rangeProofPuzzle[i][j].out);
